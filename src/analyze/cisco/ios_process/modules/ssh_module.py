...
def _get_cisco_ios_ssh_version(filename: str) -> str:
    ...
    if (len(ssh_version) > 0):
        version = host[0].re_match_typed(r'^ip ssh version\s+(\S+)', default='')
        return version
    else: 
        return None
...
def get_cisco_ios_ssh_retries(filename: str) -> str:
    ...
    if (len(retries) > 0):
        max_retries = host[0].re_match_typed(r'^ip ssh authentication-retries\s+(\S+)', default='')
        return max_retries
    else: 
        return None
...
def get_cisco_ios_ssh_timeout(filename: str) -> str:
    ...
    if (len(timeout) > 0):
        seconds = host[0].re_match_typed(r'^ip ssh time-out\s+(\S+)', default='')
        return seconds
    else: 
        return None
...
def get_cisco_ios_ssh_interface(filename: str) -> str:
    ...
    if (len(src_interface) > 0):
        interface = host[0].re_match_typed(r'^ip ssh source-interface\s+(\S+)', default='')
        return interface
    else: 
        return None
